...
//// debug only, cause loss manually, remove later
//    if(pkt_ctr == WHICH_PKT_TO_LOSE && strcmp(hostname,WHO)==0)
//    {
//        TraceableInfo(L_YELLOW("%s packet %d lost\n"),WHO,pkt_ctr);
//        pkt_ctr++;
//        return;
//    }
//    pkt_ctr++;
//// debug only end
    ...
    if (listen_socks[hashval] != NULL) {
        /// pass remote ip and local ip via listen socket->established_local_addr and established_remote_addr
        listen_socks[hashval]->established_local_addr.ip = local_ip;
        listen_socks[hashval]->established_local_addr.port = local_port;
        listen_socks[hashval]->established_remote_addr.ip = remote_ip;
        listen_socks[hashval]->established_remote_addr.port = remote_port;
...
int cal_hash(uint32_t local_ip, uint16_t local_port, uint32_t remote_ip, uint16_t remote_port) {
    ...
    if (ret < 0) { return ret + MAX_SOCK; } else { return ret; }
}

// int cal_sendtime_hash(char*data){
//     long long int ret=0;
